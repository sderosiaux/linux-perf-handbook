...
### bpftrace Inline Function Tracing
Source: FOSDEM 2025 - bpftrace: A Path to the Ultimate Linux Tracing Tool

bpftrace now uses LLDB from the LLVM ecosystem to trace inline functions. When tracing a binary with debug symbols (VM Linux or user binaries with DWARF), bpftrace can attach probes to all inline instances of a function.
...
### bpftrace Container/Namespace Support
Source: FOSDEM 2025 - bpftrace: A Path to the Ultimate Linux Tracing Tool

bpftrace now handles PID/TID helpers correctly when running inside containers with PID namespacing. The tool automatically switches between different BPF helpers depending on whether bpftrace and the traced binary are in the same namespace.

**Limitation:** Not yet supported when bpftrace is in a child namespace and the target is in the root namespace.

### bpftrace Upcoming Features
Source: FOSDEM 2025 - bpftrace: A Path to the Ultimate Linux Tracing Tool
...
### OOM Profiling with bpftrace
Source: FOSDEM 2025 - bpftrace OOM Profiler

Meta's approach to OOM debugging: intercept SIGKILL from oomd (user-space OOM killer) using bpftrace since SIGKILL cannot be intercepted in user space.
...
### eBPF Benchmarking Pitfalls
Source: FOSDEM 2025 - Five Silly Things to Do When Benchmarking Your BPF Program
...
Source: FOSDEM 2025 - Writing a Minimal Scheduler with eBPF sched_ext and C

sched_ext enables custom CPU schedulers via eBPF, available in Linux 6.12+. Previously, only 3-4 people worldwide understood kernel scheduler internals.
...
Source: FOSDEM 2025 - An Introduction to Netkit
...
### Aya: Rust eBPF Development
Source: FOSDEM 2025 - Building Your eBPF Program with Rust and Aya
...
### eBPF Verifier Performance
Source: FOSDEM 2025 - Performance Evaluation of the Linux Kernel eBPF Verifier
...
### Short-Lived Process Tracing for Security
Source: FOSDEM 2025 - Extracting Reliable Data for Short-Lived Processes Using eBPF
...
### Cilium/eBPF with Go for Kubernetes
Source: FOSDEM 2025 - An Intro to eBPF with Go
...
Source: FOSDEM 2025 - Effortless Standardised Homelab Observability with eBPF
...
Source: FOSDEM 2026 - eBPF I/O Monitoring
...
### XDP Virtual Server: Layer-2 Load Balancer
Source: FOSDEM 2026 - XDP Virtual Server

Direct Server Return (DSR) at Layer 2: load balancer only modifies MAC addresses, backend responds directly to client.
...
### eBPF Development with Nix
Source: FOSDEM 2026 - eBPF with Nix: Laptop to Testbed
...
### Container Image Profiling with eBPF
Source: FOSDEM 2026 - Podman eBPF Profiling
